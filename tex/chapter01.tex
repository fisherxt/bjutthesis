...
\end{center}
输入的双语标题会自动列入双语目录中。此外，~\verb|bitoc|宏包定义了中、英文摘要、结论章、参考文献、论文成果和致谢环境，相应环境的中英文标题会自动列入双语目录。
...
模板中的文献处理使用~\verb|gbt7714|宏包，其文献格式排版遵循国标GB/T7714-2015。\verb|gbt7714|宏包的文献引用依赖于~\verb|natbib|宏包，且已默认加载，无需额外加载命令，引用命令也与~\verb|natbib|宏包一致，模板开启了压缩文献编号选项，如遇编号连续的多篇文献会自动使用连字符压缩序号，仅保留首尾序号。
